    ...
    <BaseDialog v-model="isShow" content-class="modal-confirm-content" width="560">
        <VCard :class="$style.ModalAdmGoodsGroupDisband">
            ...
            <div key="content" :class="[$style.inner, isLoading && $style.hidden]">
                <h2 :class="$style.heading">Вы уверены что хотите удалить группу "{{ name }}"?</h2>
                <BaseList
            ...
            async handleConfirm() {
                try {
                    ...
                    await this.$notify.create({
                        message: `${this.name} была успешно удалена`,
                        type: 'positive',
                        timeout: 5000,
                    });
                    ...
                    this.isLoading = false;
                    this.$notify.create({
                        message: `${this.name} не удалось удалить`,
                        type: 'negative',
                        timeout: 5000,
                    });
