            ...
            <VFadeTransition mode="out-in" appear>
                ...
                <div v-else-if="!isSuccess" key="common" :class="$style.inner">
                    ...
                    <div :class="$style.textWrapper">
                        <h2 :class="$style.heading">Более 1000 товаров?</h2>
                        <div :class="$style.subheading">
                            Если ваш активный ассортимент больше 1000 <br />
                            товаров, оставьте заявку и мы подберём для вас <br />
...
<script>
    export default {
        name: 'ModalTariffOver10KRequest',
        data() {
            return {
                isShow: true,
                isLoading: false,
            ...
            async handleConfirm() {
                ...
                try {
                    const { success, errors } = await this.$axios.$get(
                        '/api/v1/send-mail?type=over10ksku'
                    );
                    if (!success) {
